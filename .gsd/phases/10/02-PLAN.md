...
must_haves:
  truths:
    ...
    - "Painel financeiro do Passo 4 plota gráficos de fluxo de caixa e exibe VPL, TIR e Payback corretamente"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    Salvar o resumo de fluxo de caixa e os KPIs (VPL, TIR, Payback) no arquivo Pickle (.pkl) de resultados e no config_snapshot.json.
    AVOID: Modificar a orquestração de I/O do checklist de checkpoints. Apenas adicione a consolidação no final.
  </action>
  <verify>Verificar se o config_snapshot.json gerado após simular contém chaves como "kpis_financeiros".</verify>
  <done>Manager calcula e exporta os dados de retorno financeiro.</done>
</task>
...
<task type="checkpoint:human-verify">
  ...
  <action>
    ...
    Calcular as métricas financeiras usando as configurações em cache e renderizar cards para CAPEX Total, VPL, TIR e Paybacks.
  ...
  <verify>Fazer o upload de uma simulação no Passo 4 e verificar se a aba financeira renderiza os gráficos e cartões perfeitamente.</verify>
  <done>Visualização rica integrada e testada no dashboard.</done>
</task>
...
- [ ] Streamlit exibe corretamente a aba 💰 Viabilidade Financeira com dados reais do BESS.
</verification>

<success_criteria>
- [ ] Todo o pipeline financeiro integrado sem erros.
- [ ] A interface Streamlit exibe visualização gráfica de payback e fluxo de caixa de forma interativa.
</success_criteria>
