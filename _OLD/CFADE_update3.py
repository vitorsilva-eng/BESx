...
def simular_degradacao_bateria(config):
    ...
    try:
        # Passa os parâmetros de configuração necessários
        df_1ano = carregar_dados(
            filename=cfg_arq["entrada_csv"],
            novos_nomes_colunas=cfg_arq["colunas_csv"]
        )
    except (FileNotFoundError, ValueError) as e:
        print(f"Erro ao carregar dados: {e}")
        return

    df_simulacao = multiplicar_dados(
        df_1ano, 
        tempo_simulacao_anos, 
        dt_minutos,
        cfg_sim["dias_por_ano_avg"] # Passa a constante
    )
    df_simulacao = df_simulacao.iloc[:-144] # Remove últimas 12 horas para alinhar meses
...
if __name__ == "__main__":
    CONFIGURACAO = {
        "simulacao": {
            ...
            "dias_por_ano_avg": 365.25, # Para cálculo de multiplicação
        },
        "arquivos": {
            "entrada_csv": "dadosnovos.csv", # Corrigi a extensão .cs
