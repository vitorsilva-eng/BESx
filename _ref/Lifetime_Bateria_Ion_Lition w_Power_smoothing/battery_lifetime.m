function [C_fade_cyc_sum11,C_fade_cyc_sum1,C_fade] = battery_lifetime(mission_oneyear_SOC,Tbat_oney_wo,time);
...
C_fade_csum1 = 0;
for i=1:length(SOC_cal)
    C_fade_csum1 = ((C_fade_cyc_idl(i))^(10/8) + (C_fade_csum1)^(10/8))^(0.8);
...
for i=1:length(SOC_mean)
    C_fade_csum1 = sqrt((C_fade_cyc(i))^2 + (C_fade_csum1)^2);
...
for i=1:(t_years*12)
    C_fade_1 = C_fade_cyc_sum1(time_cal*i)+C_fade_cyc_sum11(time_cyc*i);
    C_fade(i) = C_fade_1;
end
